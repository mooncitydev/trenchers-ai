...
const SEED = Array.from({ length: 8 }, genToken).map((t) => ({
  ...t,
  ageSeconds: Math.floor(10 + Math.random() * 480),
  isNew: false,
}));
...
export default function SniperPage() {
  ...
  const handleSnipe = (id: string) => {
    setTokens((prev) =>
      prev.map((t) => (t.id === id ? { ...t, sniped: true } : t)),
    );
    setSniped((n) => n + 1);
  };
  ...
  return (
    ...
    >
      <ToolNav
        rightSlot={
          <div className="hidden sm:flex items-center gap-4">
            <span
              className="text-[9px] text-[#2A3545]"
              style={{ fontFamily: "var(--font-jetbrains)" }}
            >
              SNIPED
            </span>
            <span
              className="text-[#00FF85] text-[13px] font-bold tabular-nums"
              style={{ fontFamily: "var(--font-jetbrains)" }}
            >
              {sniped}
            </span>
      ...
      <div
        className="flex items-center justify-between px-4 py-2.5 border-b border-[#111822] flex-shrink-0"
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <span
            className="text-[9px] uppercase tracking-widest text-[#2A3545]"
            style={{ fontFamily: "var(--font-jetbrains)" }}
          >
            Auto-Snipe
          </span>
          <button
            ...
            style={{
              background: autoSnipe ? "#00FF85" : "#111822",
              border: "1px solid " + (autoSnipe ? "#00FF85" : "#1C2535"),
            }}
          >
            <span
          ...
          {autoSnipe && (
            <span
              className="text-[9px] text-[#00FF85] uppercase tracking-wider animate-pulse"
              style={{ fontFamily: "var(--font-jetbrains)" }}
            >
              ● Active
            </span>
      ...
      {showFilters && (
        <div
          className="flex items-center gap-6 px-4 py-3 border-b border-[#111822] flex-shrink-0"
          style={{ background: "#07090F" }}
        >
          <div className="flex items-center gap-2">
            <span
              className="text-[9px] uppercase tracking-wider text-[#2A3545]"
              style={{ fontFamily: "var(--font-jetbrains)" }}
            >
              Min Safety
            </span>
            <div className="flex items-center gap-1">
              {[3, 5, 7, 9].map((v) => (
                <button
                  key={v}
                  onClick={() => setMinSafety(v)}
                  className="px-2 py-0.5 text-[9px] border transition-all"
                  style={{
                    fontFamily: "var(--font-jetbrains)",
                    color: minSafety === v ? "#00FF85" : "#2A3545",
                    borderColor:
                      minSafety === v ? "rgba(0,255,133,0.3)" : "#1C2535",
                    background:
                      minSafety === v ? "rgba(0,255,133,0.06)" : "transparent",
                  }}
                >
                  {v}+
                </button>
              ...
              {[5000, 10000, 25000, 50000].map((v) => (
                <button
                  ...
                  style={{
                    ...
                    borderColor:
                      minLiq === v ? "rgba(0,255,133,0.3)" : "#1C2535",
                    background:
                      minLiq === v ? "rgba(0,255,133,0.06)" : "transparent",
                  }}
                >
                  {fmtNum(v)}
                </button>
      ...
      <div
        className="hidden sm:grid px-4 py-2 border-b border-[#111822] flex-shrink-0"
        style={{
      ...
      >
        {[
          ...
          <span key={i} className={i > 1 ? "text-right" : ""}>
            {h}
          </span>
        ))}
      </div>
        ...
        {visible.map((t) => (
          ...
          >
            <div className="flex items-center gap-2.5">
              <div
                className="w-7 h-7 flex-shrink-0 flex items-center justify-center text-[10px] font-bold relative overflow-hidden"
                style={{
                  background:
                    t.safety >= 7
                      ? "rgba(0,255,133,0.12)"
              ...
              >
                ...
                {t.isNew && (
                  <div
                    className="absolute inset-0 animate-ping rounded-none"
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  <span
                    className="text-[12px] text-[#D0D8E8] font-medium"
                    style={{ fontFamily: "var(--font-jetbrains)" }}
                  >
                    ${t.name}
                  </span>
                  {t.isNew && (
                    <span
                      className="text-[8px] px-1 py-px text-[#00FF85] uppercase tracking-wider"
                      style={{
                        fontFamily: "var(--font-jetbrains)",
                  ...
                  {t.sniped && (
                    <span
                      className="text-[8px] px-1 py-px text-[#00D4FF] uppercase tracking-wider"
                      style={{
                        fontFamily: "var(--font-jetbrains)",
                ...
                <div className="flex items-center gap-1.5">
                  <span
                    ...
                    style={{
                      fontFamily: "var(--font-jetbrains)",
                      color: t.pool === "pump.fun" ? "#FF6535" : "#00D4FF",
                    }}
                  >
                    {t.pool}
                  </span>
                  ...
                  {t.lpBurned && (
                    <span
                      className="text-[8px] text-[#00FF85]"
                      style={{ fontFamily: "var(--font-jetbrains)" }}
                    >
                      LP✓
                    </span>
            ...
            <span
              ...
              style={{
                fontFamily: "var(--font-jetbrains)",
                color: t.ageSeconds < 60 ? "#FFB800" : "#5A6478",
              }}
            >
              {fmtAge(t.ageSeconds)}
            </span>
            ...
            <span
              className="text-[10px] text-[#5A6478] text-right tabular-nums hidden sm:block"
              style={{ fontFamily: "var(--font-jetbrains)" }}
            >
              {fmtNum(t.mcap)}
            </span>
            ...
            <span
              className="text-[10px] text-[#8892A4] text-right tabular-nums hidden sm:block"
              style={{ fontFamily: "var(--font-jetbrains)" }}
            >
              {fmtNum(t.liq)}
            </span>
            ...
            <span
              className="text-[10px] text-[#3A4558] text-right tabular-nums hidden sm:block"
              style={{ fontFamily: "var(--font-jetbrains)" }}
            >
              {t.holders}
            </span>
            ...
            <span
              className="text-[10px] text-right tabular-nums hidden sm:block"
              style={{
                fontFamily: "var(--font-jetbrains)",
                color: t.devHoldPct > 10 ? "#FF3B3B" : "#3A4558",
              }}
            >
              {t.devHoldPct}%
            </span>
      ...
      <div
        className="flex items-center justify-between px-4 py-2 border-t border-[#111822] flex-shrink-0"
        style={{
          background: "#07090F",
          fontFamily: "var(--font-jetbrains)",
          fontSize: "9px",
      ...
      >
        <span className="text-[#2A3545]">
          {visible.length} tokens detected · Solana Mainnet
        </span>
        <span className="text-[#00FF85]">
          {autoSnipe
            ? `● Auto-Snipe ON · min safety ${minSafety}/10 · min liq ${fmtNum(minLiq)}`
            : "● Monitoring"}
        </span>
      </div>
    </div>
