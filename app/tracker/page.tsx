...
export default function TrackerPage() {
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden" style={{ background: "#07090F" }}>
            ...
            {[
              ...
              <div key={s.label} className="text-right">
                ...
                <p className="text-[11px] font-semibold tabular-nums"
                  style={{ fontFamily: "var(--font-jetbrains)", color: s.color ?? "#E8EDF5" }}>
                  {s.value}
                </p>
              </div>
            ))}
      ...
      {/* Page title */}
      <div className="flex items-center gap-3 px-4 py-2 border-b border-[#111822] flex-shrink-0"
        style={{ background: "#080C14" }}>
        <span className="text-[9px] uppercase tracking-[0.2em] text-[#00FF85]"
          style={{ fontFamily: "var(--font-jetbrains)" }}>
          // Wallet Intelligence
        </span>
        <span className="text-[#1C2535]">·</span>
        <span className="text-[10px] text-[#2A3545]"
          style={{ fontFamily: "var(--font-jetbrains)" }}>
        ...
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <span className="text-[9px] text-[#00FF85] uppercase tracking-wider"
            style={{ fontFamily: "var(--font-jetbrains)" }}>Live</span>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Left wallet panel */}
        <div className="hidden sm:flex flex-col flex-shrink-0" style={{ width: "300px" }}>
          <WalletList
            wallets={wallets}
      ...
      {/* Mobile wallet tabs */}
      <div className="sm:hidden flex border-t border-[#111822] overflow-x-auto"
        style={{ background: "#07090F" }}>
        <button onClick={() => setSelectedWalletId(null)}
        ...
        {wallets.filter((w) => w.tracked).map((w) => (
          <button key={w.id} onClick={() => handleSelectWallet(w.id)}
