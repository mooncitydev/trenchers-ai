...
export default function TokenTradePanel({
  ...
  const handlePresetVal = (
    index: number,
    value: string,
    type: "sol" | "percentage" | "supply_pct",
  ) => {
    if (value === "") {
      if (type === "supply_pct") {
  ...
  const cycleAmountType = () => {
    ...
    setAmountInput("");
    if (amountType === "sol")
      setAmountType(isBuy ? "supply_pct" : "percentage");
  ...
  const ctaLabel = (() => {
    ...
    if (!isBuy)
      return `Sell${sym}${selectedAmount > 0 ? ` ${selectedAmount}%` : ""}`;
    ...
    if (amountType === "supply_pct") {
      ...
      return sol > 0
        ? `Buy${sym} ${formatSol(sol)}`
        : `Buy${sym} ${selectedAmount}%`;
    }
    return `Buy${sym}`;
  })();
  ...
  const renderPresetRow = () => {
    const sep = (i: number) =>
      i > 0 ? (
        <div key={`s-${i}`} className="w-px h-full bg-[#1C2535]" />
      ) : null;
            ...
            <div className="flex-1 min-w-0">
              {isEditMode ? (
                <input
                  ...
                  value={value}
                  onChange={(e) =>
                    handlePresetVal(index, e.target.value, "percentage")
                  }
                  onKeyDown={(e) => {
            ...
            <div className="flex-1 min-w-0">
              {isEditMode ? (
                <input
                  ...
                  value={value}
                  onChange={(e) =>
                    handlePresetVal(index, e.target.value, "supply_pct")
                  }
                  onKeyDown={(e) => {
                    if (e.key === "Enter") setIsEditMode(false);
                  }}
          ...
          <div className="flex gap-1">
            <button
              ...
              className={`px-2.5 h-6 text-[11px] rounded-[5px] transition-all ${
                tradingMode === "market"
                  ? "bg-trench-raised text-[#E8EDF5] font-medium"
                  : "text-trench-label hover:text-[#C8D0DC]"
              }`}
              style={{ fontFamily: "var(--font-jetbrains)" }}
            >
              Market
          ...
          <>
              ...
              <div className="flex flex-col gap-1">
                <div className="border border-trench-line rounded-t-md px-2.5 py-2 flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <span
                      className="text-[11px] text-trench-label shrink-0"
                      style={{ fontFamily: "var(--font-jetbrains)" }}
                    >
                      AMOUNT:
                    </span>
                    <input
                      ...
                      placeholder={
                        amountType === "sol" ? "Enter SOL" : "Enter %"
                      }
                    ...
                    />
                    {selectedAmount > 0 &&
                      amountType === "supply_pct" &&
                      supplyPctToSol(selectedAmount) > 0 && (
                        <span
                          className="text-[11px] text-trench-label whitespace-nowrap"
                          style={{ fontFamily: "var(--font-jetbrains)" }}
                        >
                  ...
                  <div className="shrink-0 flex items-center">
                    {amountType === "sol" ? (
                      <button
                        type="button"
                        onClick={cycleAmountType}
                        className="cursor-pointer hover:opacity-80 p-0.5"
                        title="Switch amount type"
                      >
            ...
            {isBuy && (
              <div className="px-2.5 py-3 border-b border-trench-line">
                ...
                >
                  ...
                  <span
                    className="text-sm text-trench-label"
                    style={{ fontFamily: "var(--font-dm-sans)" }}
                  >
            ...
            <div className="p-2.5 border-b border-trench-line">
              ...
              <div className="flex items-center justify-between text-[11px] mt-2 gap-1 flex-wrap">
                <span
                  className="text-trench-dim"
                  style={{ fontFamily: "var(--font-jetbrains)" }}
                >
                  Presets:
                </span>
                <div
                  className="flex items-center gap-2 text-trench-label flex-wrap justify-end"
                  style={{ fontFamily: "var(--font-jetbrains)" }}
                >
