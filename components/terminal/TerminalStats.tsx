...
export default function TerminalStats({ tokens }: Props) {
  ...
  const items = [
    ...
    { label: "Top Gainer", value: stats.topGainer?.name ?? "—", color: "#34D399" },
    { label: "Total Txns", value: stats.totalTxns.toLocaleString(), color: "#22D3EE" },
  ];
      ...
      {items.map((item, i) => (
        <div key={item.label} className="flex items-center">
          <div className="flex flex-col px-4 sm:px-5 py-3 flex-shrink-0 min-w-[100px]">
            ...
            <span className="text-[13px] font-semibold tabular-nums tracking-tight" style={{ fontFamily: "var(--font-jetbrains)", color: item.color }}>
              {item.value}
            </span>
          </div>
          {i < items.length - 1 && <div className="w-px h-9 bg-trench-line-subtle flex-shrink-0" />}
        </div>
      ))}
    </div>
  );
