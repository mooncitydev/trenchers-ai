...
const TOOLS: { href: string; id: ToolId; label: string; short: string }[] = [
  ...
  { href: "/app/portfolio", id: "portfolio", label: "Portfolio", short: "Port" },
    ...
    <header className="flex flex-col border-b border-trench-line-subtle flex-shrink-0 bg-trench-bg/95 backdrop-blur-sm">
      <div className="flex items-center justify-between px-3 sm:px-5 h-12 sm:h-[52px] gap-3">
        <div className="flex items-center gap-2 sm:gap-4 min-w-0">
          ...
          >
            <svg width="14" height="14" viewBox="0 0 16 16" fill="none" aria-hidden className="opacity-70">
          ...
          <div className="flex items-center gap-2 min-w-0">
            <div
              ...
              aria-hidden
            >
              <span className="text-[11px] font-bold text-trench-accent" style={{ fontFamily: "var(--font-jetbrains)" }}>
                T
              </span>
            </div>
            <nav
        ...
        <div className="text-right min-w-0 pl-2 border-l border-trench-line-subtle sm:border-0 sm:pl-0">
          <p className="text-[#E8EDF5] text-xs sm:text-sm font-medium truncate tracking-tight" style={{ fontFamily: "var(--font-dm-sans)" }}>
            {title ?? "TrenchersAI"}
          </p>
          {subtitle && (
            <p className="text-[10px] text-trench-dim truncate mt-0.5" style={{ fontFamily: "var(--font-jetbrains)" }}>
              {subtitle}
            </p>
          )}
        </div>
