  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 flex-wrap">
        <span
          className="text-trench-dim uppercase tracking-[0.14em] shrink-0"
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {PILLARS.map((p, i) => (
            <span key={p.k} className="flex items-center gap-1.5">
              {i > 0 && (
                <span
                  className="text-trench-line w-px h-3 hidden sm:inline"
                  aria-hidden
                />
              )}
              <abbr title={p.hint} className="cursor-help no-underline">
                <span
                  className="text-trench-accent font-semibold"
                  style={{ fontFamily: "var(--font-jetbrains)" }}
                >
                  {p.label}
                </span>
      ...
      <div
        className="flex items-center gap-3 sm:gap-4 text-trench-label"
        style={{ fontFamily: "var(--font-jetbrains)" }}
      >
        ...
        <span className="text-trench-dim sm:text-trench-label leading-snug">
          ...
          <span className="hidden sm:inline">
            Prototype UI · data simulated · architecture production-shaped
          </span>
        </span>
      </div>
    </motion.div>
