...
import PortfolioNavigation, {
  type PortfolioMainTab,
} from "@/components/portfolio/PortfolioNavigation";
...
function PnLSparkline() {
  const points =
    "0,80 40,72 80,55 120,60 160,35 200,40 240,22 280,18 320,25 360,10";
  return (
    <div className="border border-trench-line rounded-lg bg-trench-bg p-3 h-[140px] flex flex-col shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
      <div className="flex items-center justify-between mb-2">
        <span
          className="text-[11px] text-trench-label"
      ...
      <p
        className="text-[9px] text-trench-dim mt-1"
        style={{ fontFamily: "var(--font-jetbrains)" }}
      >
  ...
  const SortBtn = ({
    ...
    <button
      ...
      className={`inline-flex items-center gap-1 text-[9px] font-semibold uppercase tracking-wider transition-colors ${
        sortKey === k
          ? "text-amber-400"
          : "text-trench-dim hover:text-trench-label"
      }`}
      style={{ fontFamily: "var(--font-jetbrains)" }}
    >
      {children}
          ...
          <div className="max-w-6xl mx-auto p-4 sm:p-6 space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <option value="all">
                    All wallets ({MOCK_WALLETS.length})
                  </option>
                  {MOCK_WALLETS.map((w) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="border border-trench-line rounded-lg bg-trench-panel p-4 space-y-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
                <p
                  className="text-sm text-[#E8EDF5] font-medium"
                  style={{ fontFamily: "var(--font-dm-sans)" }}
                >
                  Balance
                </p>
                {[
                  {
                    label: "Total value",
                    v: PORTFOLIO_SUMMARY.totalValue,
                    color: "#E8EDF5",
                  },
                  {
                    label: "Unrealized PnL",
                    v: PORTFOLIO_SUMMARY.unrealizedPnL,
                    signed: true,
                  },
                  {
                    label: "Realized PnL",
                    v: PORTFOLIO_SUMMARY.realizedPnL,
                    signed: true,
                  },
                  {
                    label: "Available balance",
                    v: PORTFOLIO_SUMMARY.availableBalance,
                    color: "#E8EDF5",
                  },
                ].map((row) => (
                  <div key={row.label}>
                    <div
                      className="text-[10px] text-trench-dim mb-0.5"
                      style={{ fontFamily: "var(--font-jetbrains)" }}
                    >
                      {row.label}
                    </div>
                    ...
                    >
                      {"signed" in row && row.signed
                        ? `${row.v >= 0 ? "+" : ""}${formatUsd(row.v)}`
                        : formatUsd(row.v as number)}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="relative max-w-xs">
                ...
                />
                <span
                  className="absolute left-2.5 top-1/2 -translate-y-1/2 text-trench-dim text-[10px]"
                  aria-hidden
                >
                  ⌕
                </span>
            ...
            <div className="border border-trench-line rounded-lg overflow-hidden bg-trench-panel shadow-[inset_0_1px_0_rgba(255,255,255,0.02)]">
              {spotSub === "trade-history" ? (
                <table className="w-full text-left">
                  <thead>
                    <tr
                      className="bg-trench-raised border-b border-trench-line-subtle text-[9px] uppercase text-trench-dim"
                      style={{ fontFamily: "var(--font-jetbrains)" }}
                    >
                  ...
                  <tbody>
                    {MOCK_TRADES.map((t) => {
                      ...
                      return (
                        <tr
                          key={t.id}
                          className="border-b border-trench-line-subtle hover:bg-trench-raised-hover/90"
                        >
                          <td
                            className="px-3 py-2.5 text-[11px] text-[#C8D0DC]"
                            style={{ fontFamily: "var(--font-jetbrains)" }}
                          >
                          ...
                          <td className="px-3 py-2.5">
                            <div className="flex items-center gap-2">
                              <TokenAvatar
                                imageIndex={stableImageIndex(t.symbol)}
                                fallbackLetter={t.symbol}
                                size="sm"
                                up={t.pnlUsd == null || (t.pnlUsd ?? 0) >= 0}
                              />
                              <div>
                                <div
                                  className="text-[11px] text-[#E8EDF5]"
                                  style={{
                                    fontFamily: "var(--font-jetbrains)",
                                  }}
                                >
                                ...
                                <div
                                  className="text-[9px] text-trench-dim truncate max-w-[88px]"
                                  style={{
                                    fontFamily: "var(--font-jetbrains)",
                                  }}
                                >
                          ...
                          <td
                            className="px-3 py-2.5 text-[11px] text-trench-label tabular-nums"
                            style={{ fontFamily: "var(--font-jetbrains)" }}
                          >
                            {t.amount}
                          </td>
                          <td
                            className="px-3 py-2.5 text-[11px] tabular-nums text-[#C8D0DC]"
                            style={{ fontFamily: "var(--font-jetbrains)" }}
                          >
                            {val(t.valueUsd)}
                          </td>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-trench-raised border-b border-trench-line-subtle">
                      ...
                      <th
                        className="px-3 py-2.5 text-right text-[9px] uppercase text-trench-dim font-semibold"
                        style={{ fontFamily: "var(--font-jetbrains)" }}
                      >
                        Action
                      </th>
                  ...
                  <tbody>
                    {(spotSub === "top-100" ? top100 : filteredPositions).map(
                      (p, i) => (
                        <tr
                          key={p.id}
                          className="border-b border-trench-line-subtle hover:bg-trench-raised-hover/90"
                        >
                          <td className="px-3 py-2.5">
                            <div className="flex items-center gap-2">
                              {spotSub === "top-100" && (
                                <span
                                  className="text-[10px] text-trench-dim w-5 tabular-nums"
                                  style={{
                                    fontFamily: "var(--font-jetbrains)",
                                  }}
                                >
                              ...
                              <TokenAvatar
                                imageIndex={stableImageIndex(p.symbol)}
                                fallbackLetter={p.symbol}
                                size="sm"
                                up={p.pnlUsd >= 0}
                              />
                              <div>
                                <div
                                  className="text-[11px] text-[#E8EDF5] font-medium"
                                  style={{
                                    fontFamily: "var(--font-jetbrains)",
                                  }}
                                >
                                ...
                                <div
                                  className="text-[9px] text-trench-dim truncate max-w-[120px]"
                                  style={{
                                    fontFamily: "var(--font-jetbrains)",
                                  }}
                                >
                          ...
                          <td
                            className="px-3 py-2.5 text-right text-[11px] tabular-nums text-trench-label"
                            style={{ fontFamily: "var(--font-jetbrains)" }}
                          >
                            {val(p.bought)}
                          </td>
                          ...
                          <td
                            className="px-3 py-2.5 text-right text-[11px] tabular-nums text-[#C8D0DC]"
                            style={{ fontFamily: "var(--font-jetbrains)" }}
                          >
                            {val(p.remaining)}
                          </td>
